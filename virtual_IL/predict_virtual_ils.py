...
TEMPS = [298.15, 323.15, 348.15, 368.15]
P = 0.1  # MPa
...
spec = importlib.util.spec_from_file_location(
    "gin_tc", os.path.join(ROOT, "ThermalConductivity_program", "src", "models", "gnn", "gin.py"))
...
def batch_predict(model, graphs, cond, name):
    ...
    for start in range(0, len(valid_idx), bs):
        ...
        n = len(glist)
        cond_b = cond.repeat(n, 1).to(DEVICE) if name == "lambda" else cond.repeat(n).to(DEVICE)
        with torch.no_grad():
...
def main():
    ...
    il_smiles_list, cat_smiles_list, an_smiles_list = [], [], []
    ...
    print(f"  {n_valid}/{n_il} valid  ({n_il - n_valid} failed)")
    ...
    for t in TEMPS:
        ...
        for i in range(n_il):
            ...
            damb = 2 * d_plus * d_minus / (d_plus + d_minus) \
                if not (np.isnan(d_plus) or np.isnan(d_minus)) else np.nan
            rows.append({
                ...
                "D-": d_minus,
                "Damb": damb,
                "sigma": 10 ** all_preds["sigma"][i],
                "lambda": all_preds["lambda"][i],
            })

    df = pd.DataFrame(rows)
    out_cols = ["IL_smiles", "cation_smiles", "anion_smiles", "T",
                "D+", "D-", "Damb", "sigma", "lambda"]
    df[out_cols].to_csv(os.path.join(DIR, "virtual_IL_predictions.csv"), index=False)

    print("\nPrediction summary:")
    for t in TEMPS:
        sub = df[df["T"] == t]
        n_nan = sub.isnull().any(axis=1).sum()
        print(f"  T={t}K: {len(sub)} ILs  ({n_nan} with NaN)")
        for col in ["Damb", "sigma", "lambda"]:
            v = sub[col].dropna()
            print(f"    {col}: min={v.min():.4f}, max={v.max():.4f}, mean={v.mean():.4f}")

    print(f"\nSaved: virtual_IL_predictions.csv  ({len(df)} rows)")
